...
# %% Build graph of countries & use four-colour theorem
water = '#6baed6'
...
g = svgwrite.container.Group(id='countries', opacity=0.25)
for continent, fill in olympic_colors.items():
    ...
    for row in gdf[gdf['CONTINENT'] == continent].itertuples():
        if four_colour:
            fill = 'white' if continent == "Antarctica" else color_map[colors[row.Index]]
